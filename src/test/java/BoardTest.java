...
public class BoardTest {
    ...
    public void testInitMid() {
        ...
        for (int y = 1; y < board.getSize() - 1; y++) {
            for (int x = 0; x < board.getSize(); x++) {
                assert(board.getSquareAtPos(x, y).getState().equals(Square.State.EMPTY));
            }
        }
    }
    @Test
    ...
    public void testInitMidShrine() {
        assert(board.getSquareAtPos(2, 0).isShrine());
        assert(board.getSquareAtPos(2, 4).isShrine());
    }
    ...
    public void testPlayPieceValid() {
        ...
        int x = 0;
        int y = 0;
        assert(board.playPiece(0,4, x, y));
        ...
        int yy = 3;
        assert(board.playPiece(1,4, x, yy));
    ...
    public void testDetectWinnerOnShrine() {
        ...
        board.playPiece(2, 4, 2, 3);
        assert(board.getWinner() == null);
        board.otherPlayerTurn();
        board.playPiece(0, 0, 2, 4);
        assert(!(board.getWinner() == null));
    ...
    public void testDetectWinnerTakeoutMaster() {
        ...
        board.playPiece(2, 4, 1, 2);
        assert(board.getWinner() == null);
        board.otherPlayerTurn();
        board.playPiece(0, 1, 1, 2);
        assert(!(board.getWinner() == null));
        assert(board.getWinner().getTeam().equals(Player.Team.BLUE));
    }
